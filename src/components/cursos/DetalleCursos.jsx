  ...
  useEffect(() => {
    const obtenerInformacionCurso = async () => {
      try {
        ...
        const instructorResponse = await axios.get(`http://localhost:3000/instructor/detalle/${response.data.instructor}`);
        setInstructor(instructorResponse.data);

        const usuario = JSON.parse(localStorage.getItem('usuario'));
        if (usuario && usuario.user) {
          setIdUsuario(usuario.user._id);
        }
      } catch (error) {
        console.error('Error fetching course or instructor data', error);
      }
    };
